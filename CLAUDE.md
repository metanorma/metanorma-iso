...
pubid 2 is a rewrite: flavor modules with typed-component models, no `Identifier.create`, no universal `Registry.parse`, no `Pubid::Core::Errors`. Key API used here (`lib/metanorma/iso/front_id.rb`):
- Construction: flavor classes, e.g. `Pubid::Iso::Identifiers::InternationalStandard.new(number:, part:, date:, publisher:, copublishers:, languages:, typed_stage:)`. Type class via `Pubid::Iso.locate_type(code)`; typed stage via `Pubid::Iso.all_typed_stages.find { harmonized_stages.include?(code) && type_code == tc }` (60.60 = published: abbr "IS"/"TR"/...; visible abbreviation is the FIRST NON-EMPTY variant, e.g. published IS is `["", "IS"]`).
- Rendering: `id.to_s` (typed stages always shown; language form comes from the Language component's `original_code` — single char "E" for ref_num_short, ISO code "en" for ref_num_long); `id.to_urn` (2.x URN grammar: stage ABBR like `WD.3`/`FDIS`/`FDTS`, published stages omitted, versions as `v1.3`, explicit `:en`).
- `id.stage` is COMPUTED from `typed_stage` (`typed_stage.to_stage`) — never assign it.
- Errors: `Parslet::ParseFailed` (+ `ArgumentError` for over-long input); the 1.x `Pubid::Core::Errors` hierarchy is gone. Illegal stage codes must be raised explicitly (ISO_9 warning depends on it).
- CEN: `Pubid::CenCenelec` (e.g. `Identifiers::EuropeanNorm`, `AdoptedEuropeanNorm` with `adopted_identifier`).
- Known upstream gaps: pubid 2's grammar rejects the undated `:--` form (isodoc's `std_docid_semantic` regex-fallback covers annotation for those); `Pubid::Iso::Parser::ParseError` and `Pubid::Errors::ParseError` are referenced but not defined in pubid 2.0.0.pre.alpha.8.
