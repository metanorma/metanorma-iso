...
via the html2doc MHT pipeline.
* Microsoft Word output (`.docx`), a native OOXML format produced directly from
the XML model via the Uniword builder, with no HTML intermediate.
...
$ metanorma --type iso --extensions docx a.adoc  # output DOCX (native OOXML)
$ metanorma --type iso --extensions html a.adoc  # output just HTML
$ metanorma --type iso --extensions doc a.adoc   # output just DOC (MHT)
$ metanorma --type iso --extensions xml a.adoc   # output ISO XML
----

To specify output formats in an AsciiDoc document, use the `:mn-output-extensions:` attribute:

[source,asciidoc]
----
:mn-output-extensions: xml,html,docx,pdf
----

==== DOCX template selection

The DOCX output supports two ISO Word templates, controlled by the `:iso-word-template:` attribute:

[horizontal]
DIS:: `:iso-word-template: dis` — DIS/FDIS template (461 styles, for stages 40–60 and 90)
Simple:: `:iso-word-template: simple` — Simple template (72 styles, for stages 00–30)

If not set, the template is auto-detected from the document stage.
Both templates are bundled in `data/iso-dis/` and `data/iso-simple/`.
...
The gem then converts the XML into HTML, DOC, and DOCX.
