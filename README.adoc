...
Reviewer notes are encoded as sidebars. The following attributes on reviewer
notes are mandatory:

* reviewer attribute (naming the reviewer) 
* the starting target anchor of the note (as a `from` attribute)

The following attributes are optional:

* date attribute, optionally including the time (as xs:date or xs:datetime)
* the ending target anchor of the note (as a `to` attribute)
...
[[para1]]
This is a clause address profile definition
