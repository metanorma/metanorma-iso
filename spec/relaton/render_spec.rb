...
RSpec.describe Relaton::Render::Iso do
  ...
  it "renders software" do
    ...
    INPUT
    output = <<~OUTPUT
      <formattedref>RIBOSE INC. <em>metanorma-standoc</em>. Version 1.3.1. 2019. Available at: <span class='biburl'><fmt-link target='https://github.com/metanorma/metanorma-standoc'>https://github.com/metanorma/metanorma-standoc</fmt-link></span></formattedref>
    OUTPUT
    p = renderer
    expect(p.render(input))
      .to be_equivalent_to output
  ...
  it "renders external standard, IETF" do
    ...
    INPUT
    output = <<~OUTPUT
      <formattedref>S. BRADNER. <em><span class='stddocTitle'>Intellectual Property Rights in IETF Technology</span></em>. RFC Series</formattedref>
    OUTPUT
    p = renderer
    expect(p.render(input))
      .to be_equivalent_to output
  ...
  it "renders external standard, W3C" do
    ...
    INPUT
    output = <<~OUTPUT
      <formattedref><em><span class='stddocTitle'>Time Ontology in OWL</span></em>. Recommendation</formattedref>
    OUTPUT
    p = renderer
    expect(p.render(input))
      .to be_equivalent_to output
  ...
  it "renders website" do
    ...
    INPUT
    output = <<~OUTPUT
      <formattedref>LIBERMAN, M. and G. PULLUM. <em><span class='stddocTitle'>Language Log</span></em> [website]. University of Pennsylvania. 2003–. Available at: <span class='biburl'><fmt-link target='https://languagelog.ldc.upenn.edu/nll/'>https://languagelog.ldc.upenn.edu/nll/</fmt-link></span>. [viewed: September 3, 2019]</formattedref>
    OUTPUT
    p = renderer
    expect(p.render(input))
      .to be_equivalent_to output
  ...
  it "renders unpublished" do
    ...
    INPUT
    output = <<~OUTPUT
      <formattedref>JENKINS and J. RUOSTEKOSKI. <em>Controlled manipulation of light by cooperativeresponse of atoms in an optical lattice</em>. Preprint. 2012. Available at: <span class='biburl'><fmt-link target='https://eprints.soton.ac.uk/338797/'>https://eprints.soton.ac.uk/338797/</fmt-link></span>. [viewed: June 24, 2020]</formattedref>
    OUTPUT
    p = renderer
    expect(p.render(input))
      .to be_equivalent_to output
  ...
  it "renders month-year dates" do
    ...
    INPUT
    output = <<~OUTPUT
      <formattedref>JENKINS and J. RUOSTEKOSKI. <em>Controlled manipulation of light by cooperativeresponse of atoms in an optical lattice</em>. Preprint. 2020. Available at: <span class='biburl'><fmt-link target='https://eprints.soton.ac.uk/338797/'>https://eprints.soton.ac.uk/338797/</fmt-link></span>. [viewed: June 2020]</formattedref>
    OUTPUT
    p = renderer
    expect(p.render(input))
      .to be_equivalent_to output
