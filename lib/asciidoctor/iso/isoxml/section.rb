  ...
  module ISO
    module Blocks
      ...
      def anchor_or_uuid(node)
        node.id.nil? ? uuid :
          node.id.empty? ? uuid : node.id
      end

      def stem(node)
        stem_attributes = { anchor: anchor_or_uuid(node) }
      ...
      def termnote(node)
        note_attributes = { anchor: anchor_or_uuid(node) }
      ...
      def note(node)
        noko do |xml|
          xml.note **attr_code(anchor: anchor_or_uuid(node)) do |xml_cref|
            if node.blocks?
              xml_cref << node.content
            else
      ...
      def section(node)
        attrs = { anchor: anchor_or_uuid(node) }
        noko do |xml|
