  ...
  module Iso
    module Docx
      ...
      module ModelUtils
        ...
        # Read a single attribute from +node+ if the attribute is
        # declared on its class. Returns nil otherwise. Avoids
        # NoMethodError when callers walk heterogeneous model trees.
        ...
        # Read a collection-valued attribute from +node+. Always returns
        # an Array (empty when the attribute is missing or nil).
        def attribute_collection(node, attr)
          value = attribute_value(node, attr)
          Array(value)
        end
