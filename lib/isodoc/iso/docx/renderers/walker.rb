    ...
    module Docx
      module Renderers
        ...
        class Walker
          ...
          # Dispatch a single +node+ through the configured dispatcher.
          # Use when a renderer needs to send one specific child (e.g.,
          # a formula's +:dl+ or +:p+) through the normal dispatch path
          # rather than walking all of a node's children.
          def dispatch(node, doc)
            @dispatcher.call(node, doc)
          end

          # Walk every element in a collection-valued attribute (e.g.,
          # +formula.dl+, +formula.p+). No-op if the attribute is missing
          # or nil.
          def walk_attribute(node, attr, doc)
            ...
            Array(value).each { |child| @dispatcher.call(child, doc) }
          end
