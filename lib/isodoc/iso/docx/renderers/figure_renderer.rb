    ...
    module Docx
      module Renderers
        ...
        class FigureRenderer
          ...
          # Era C: figure key lists (a <dl> or <key> child) render as
          # KeyTitle + KeyText paragraphs via DefinitionListRenderer.
          # The :figure zone set by Context#with_figure routes
          # DefinitionListRenderer into its key-list pattern.
          def render_key_list(figure, doc)
            render_inline_dl(figure, doc)
            render_key_element(figure, doc)
          end

          def render_inline_dl(figure, doc)
            return unless figure.class.attributes.key?(:dl)
            return unless figure.dl

            @walker.walk(figure.dl, doc)
          end

          def render_key_element(figure, doc)
            return unless figure.class.attributes.key?(:key)
            return unless figure.key

            @walker.walk(figure.key, doc)
          end
