    ...
    module Docx
      ...
      class CorePropertiesBuilder
        ...
        def build_date_property(value, type_class)
          ...
          normalized = normalize_w3cdtf(value)
          type_class.new(value: normalized, type: W3CDTF_TYPE)
        end

        # W3CDTF requires YYYY-MM-DDThh:mm:ssZ; bare dates or non-UTC offsets
        # trip Word's strict XSD dateTime validation.
        def normalize_w3cdtf(value)
          parsed = Time.iso8601(value.to_s) rescue nil
          return parsed.getutc.strftime("%Y-%m-%dT%H:%M:%SZ") if parsed

          Date.parse(value.to_s).strftime("%Y-%m-%dT00:00:00Z")
