  ...
  module Iso
    module Docx
      ...
      class DocumentProperties
        ...
        DEFAULT_STAGE_ABBREVIATION = "IS"

        def initialize(doc_model, template: :dis)
          @model = doc_model
        ...
        def release_version
          ...
          stage_abbreviations[stage / 10 * 10] || DEFAULT_STAGE_ABBREVIATION
        end
        ...
        def ident_doc_type
          ...
          doctype_abbreviations[doctype] || doctype.upcase
        end
        ...
        def doctype_abbreviations
          @doctype_abbreviations ||= document_metadata.fetch("doctype_abbreviations", {})
        end

        def stage_abbreviations
          @stage_abbreviations ||= begin
            raw = document_metadata.fetch("stage_abbreviations", {})
            raw.each_with_object({}) { |(k, v), h| h[k.to_i] = v }
          end
        end

        def document_metadata
          @document_metadata ||= begin
            path = DocxTemplates.document_metadata_path(@template)
