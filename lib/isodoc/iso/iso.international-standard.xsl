<?xml version="1.0" encoding="UTF-8"?><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:iso="https://www.metanorma.org/ns/iso" xmlns:mathml="http://www.w3.org/1998/Math/MathML" xmlns:xalan="http://xml.apache.org/xalan" xmlns:fox="http://xmlgraphics.apache.org/fop/extensions" xmlns:pdf="http://xmlgraphics.apache.org/fop/extensions/pdf" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:java="http://xml.apache.org/xalan/java" xmlns:barcode="http://barcode4j.krysalis.org/ns" exclude-result-prefixes="java" version="1.0">
																				...
																				<xsl:when test="iso:abbreviation = 'ISO'">
																					<xsl:choose>
																						<xsl:when test="$layoutVersion = '1989' and $revision_date_num &lt; 19930101">
																							<fo:external-graphic src="{concat('data:image/png;base64,', normalize-space($Image-ISO-Logo-1989))}" content-height="{$content-height}mm" content-width="scale-to-fit" scaling="uniform" fox:alt-text="Image ISO Logo"/>
																						</xsl:when>
																						<xsl:otherwise>
	...
	<xsl:template match="iso:title" name="title">
		...
		<xsl:variable name="font-size">
			<xsl:choose>
				<xsl:when test="$layoutVersion = '1989'">
					<xsl:choose>
	...
	<xsl:template match="iso:foreword |            iso:foreword//iso:clause |            iso:introduction |            iso:introduction//iso:clause |            iso:sections//iso:clause |             iso:annex |             iso:annex//iso:clause |             iso:references[not(@hidden = 'true')] |            iso:bibliography/iso:clause |             iso:colophon |             iso:colophon//iso:clause |             *[local-name()='sections']//*[local-name()='terms'] |             *[local-name()='sections']//*[local-name()='definitions'] |            *[local-name()='annex']//*[local-name()='definitions']" mode="flatxml"> <!-- [not(@type = 'national-foreword')] [not(@type = 'national-foreword')] -->
		<!-- From:
		<clause>
			<title>...</title>
			<p>...</p>
	...
	<xsl:variable name="font-size_header">
		<xsl:choose>
			<xsl:when test="$layoutVersion = '1989'">11pt</xsl:when>
			<xsl:otherwise>12pt</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	...
	<xsl:variable name="font-size_footer_copyright">
		<xsl:choose>
			<xsl:when test="$layoutVersion = '1989'">8pt</xsl:when>
					...
					<fo:block-container>
						<fo:block font-size="12pt" font-weight="bold" padding-top="3.5mm" padding-bottom="0.5mm">
							...
							<xsl:for-each select="/iso:iso-standard/iso:bibdata/iso:ext/iso:ics/iso:code">
								...
								<xsl:if test="position() != last()">
									<xsl:choose>
										<xsl:when test="$layoutVersion = '1989'"><xsl:text>: </xsl:text></xsl:when>
										<xsl:otherwise><xsl:text>; </xsl:text></xsl:otherwise>
									</xsl:choose>
								</xsl:if>
							</xsl:for-each>
	...
	<xsl:variable name="Image-ISO-Logo-1989">
